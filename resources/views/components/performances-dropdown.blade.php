...
<div x-data="{ open: false }" x-cloak>
    ...
    <div x-show="open" x-collapse x-transition @click.away="open = false" class="mt-4">
        <div class="max-h-96 overflow-y-auto pr-4 -mr-4"> <!-- SCROLLS INTERNALLY -->
            <div class="ml-8 pl-6 border-l-2 border-gray-300 space-y-3">
                @forelse(\App\Models\Performance::orderBy('title')->get() as $perf)
                    <a href="{{ route('performances.show', $perf) }}"
                       class="block uppercase text-sm text-gray-300 hover:text-black transition py-1">
                        {{ $perf->title }}
                    </a>
                @empty
